...
def desaturate_red(array):
    # Assuming the input array has shape (num_pixels, 3) for RGB
    red_channel = array[:, 0]  # Extracting the red channel

    # Desaturate the red channel by scaling it (e.g., setting it to half)
    desaturated_red_channel = red_channel * 0.2

    # Replace the original red channel with the desaturated version
    array[:, 0] = desaturated_red_channel
...
def process_frame(frame_resized):
    ...
    numpy_d1_tr_desaturated = desaturate_red(numpy_mapped_pixels_modified_d1.copy())
    numpy_d2_tr_desaturated = desaturate_red(numpy_mapped_pixels_modified_d2.copy())
    numpy_d3_tr_desaturated = desaturate_red(numpy_mapped_pixels_modified_d3.copy())
    numpy_d4_tr_desaturated = desaturate_red(numpy_mapped_pixels_modified_d4.copy())
    numpy_d5_tr_desaturated = desaturate_red(numpy_mapped_pixels_modified_d5.copy())
    numpy_d6_tr_desaturated = desaturate_red(numpy_mapped_pixels_modified_d6.copy())

    numpy_d1_tr = np.rot90(numpy_d1_tr_desaturated.reshape((16, 16, 3)))
    numpy_d2_tr = np.rot90(numpy_d2_tr_desaturated.reshape((16, 16, 3)))
    numpy_d3_tr = np.rot90(numpy_d3_tr_desaturated.reshape((16, 16, 3)))
    numpy_d4_tr = np.rot90(numpy_d4_tr_desaturated.reshape((16, 16, 3)))
    numpy_d5_tr = np.rot90(numpy_d5_tr_desaturated.reshape((16, 16, 3)))
    numpy_d6_tr = np.rot90(numpy_d6_tr_desaturated.reshape((16, 16, 3)))
    ...
    mapped_pixels_modified_output = np.concatenate((mapped_pixels_modified_d6_out, mapped_pixels_modified_d4_out, mapped_pixels_modified_d3_out, mapped_pixels_modified_d1_out, mapped_pixels_modified_d2_out, mapped_pixels_modified_d5_out), axis=0)
